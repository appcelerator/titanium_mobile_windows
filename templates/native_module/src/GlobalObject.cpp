...
#pragma warning(pop)
	...
	GlobalObject::GlobalObject(const JSContext& js_context) TITANIUM_NOEXCEPT
	    : Titanium::GlobalObject(js_context)
			, native_module_loader__(std::make_shared<NativeModuleLoader>())
	{
		TITANIUM_LOG_DEBUG("GlobalObject::ctor");

		native_module_loader__->preload(js_context);
	}
	...
	{
		TITANIUM_LOG_WARN("GlobalObject::requiredCommonJSModuleExists: unpimplemented");
		return false;
	}

	JSValue GlobalObject::requireCommonJSModule(const JSContext& js_context, const std::string& moduleId) 
	{
		TITANIUM_LOG_WARN("GlobalObject::requireCommonJSModule: unpimplemented");
	...
	{
		return native_module_loader__->exists(js_context, moduleId);
	}

	JSValue GlobalObject::requireNativeModule(const JSContext& js_context, const std::string& moduleId) 
	{
		return native_module_loader__->load(js_context, moduleId);
	}
