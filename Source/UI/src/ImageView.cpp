      ...
      image__(ref new Windows::UI::Xaml::Controls::Image())
{
	...
	internal_load_event_ = image__->ImageOpened +=
	    ref new RoutedEventHandler([this](::Platform::Object ^ sender, RoutedEventArgs ^ e) {
		    auto rect = Titanium::LayoutEngine::RectMake(Canvas::GetLeft(this->image__), Canvas::GetTop(this->image__),
		                                                 this->image__->ActualWidth, this->image__->ActualHeight);
...
bool ImageView::setImageArgumentValidator(const JSValue& argument) TITANIUM_NOEXCEPT
{
	...
	auto uri =
	    ref new Windows::Foundation::Uri(ref new Platform::String(std::wstring(path.begin(), path.end()).c_str()));
