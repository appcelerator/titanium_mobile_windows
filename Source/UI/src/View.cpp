...
namespace UI
{
View::View(const JSContext& js_context) TITANIUM_NOEXCEPT : Titanium::UI::View(js_context),
...
void View::enableEvent(const std::string& event_name) TITANIUM_NOEXCEPT
{
	...
	if (event_name == "touchmove") {
		if (touch_move_event_count_ == 0) {
			...
			touch_move_event_ = getComponent()->ManipulationDelta += ref new ManipulationDeltaEventHandler(
			    [ctx, this](Platform::Object ^ sender, ManipulationDeltaRoutedEventArgs ^ e) {
