...
namespace TitaniumWindows
{
	namespace UI
	{
		...
		void WindowsModule::JSExportInitialize() 
		{
			...
			TITANIUM_ADD_CONSTANT_PROPERTY(WindowsModule, ListView);
			...
			TITANIUM_ADD_FUNCTION(WindowsModule, createListView);
		}
		...
		TITANIUM_PROPERTY_GETTER(WindowsModule, ListView)
		{
			return get_context().CreateObject(JSExport<TitaniumWindows::UI::WindowsXaml::ListView>::Class());
		}
		...
		TITANIUM_FUNCTION(WindowsModule, createListView)
		{
			ENSURE_OPTIONAL_OBJECT_AT_INDEX(parameters, 0);
			CREATE_TITANIUM_UI_WINDOWS(ListView);
		}
