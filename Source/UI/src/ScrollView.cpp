	...
	namespace UI
	{
		...
		ScrollView::ScrollView(const ScrollView& rhs, const std::vector<JSValue>& arguments) TITANIUM_NOEXCEPT
		    : Titanium::UI::ScrollView(rhs, arguments),
		      scroll_viewer__(ref new Windows::UI::Xaml::Controls::ScrollViewer()),
		      contentView__(
		          get_context().CreateObject(JSExport<TitaniumWindows::UI::View>::Class()).CallAsConstructor())
		{
		...
		{
			auto content =
			    std::dynamic_pointer_cast<TitaniumWindows::UI::View>(contentView__.GetPrivate<Titanium::UI::View>());
		...
		bool ScrollView::setBackgroundColorArgumentValidator(const JSValue& argument) TITANIUM_NOEXCEPT
		{
			...
			TITANIUM_LOG_INFO("ScrollView::setBackgroundColorArgumentValidator: backgroundColor = ",
			                  backgroundColorName);
		...
		{
			scroll_viewer__->ChangeView(ref new Platform::Box<double>(x), ref new Platform::Box<double>(y), nullptr,
			                            false);
		}

		void ScrollView::scrollToBottom()
		{
			scroll_viewer__->ChangeView(nullptr, ref new Platform::Box<double>(scroll_viewer__->ScrollableHeight),
			                            nullptr);
		}
		...
		{
			return scroll_viewer__->HorizontalScrollBarVisibility !=
		...
		bool ScrollView::setShowHorizontalScrollIndicator(bool enabled) TITANIUM_NOEXCEPT
		{
			if (enabled) {
				...
				scroll_viewer__->HorizontalScrollBarVisibility =
				    Windows::UI::Xaml::Controls::ScrollBarVisibility::Hidden;
			}
			return true;
		}
		...
		{
			return scroll_viewer__->VerticalScrollBarVisibility !=
			       Windows::UI::Xaml::Controls::ScrollBarVisibility::Hidden;
		}
