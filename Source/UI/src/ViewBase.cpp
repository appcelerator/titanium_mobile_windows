...
namespace TitaniumWindows
{
	namespace UI
	{
		...
		static void onLayoutCallback(Titanium::LayoutEngine::Node* node)
		{
			...
			auto rect = Titanium::LayoutEngine::RectMake(node->element.measuredLeft, node->element.measuredTop,
			                                             node->element.measuredWidth, node->element.measuredHeight);
		...
		void ViewBase::setComponent(Windows::UI::Xaml::FrameworkElement ^ component)
		{
			...
			loaded_event_ = component_->Loaded +=
			    ref new RoutedEventHandler([this](Platform::Object ^ sender, RoutedEventArgs ^ e) {
				    auto component = getComponent();
				    auto rect = Titanium::LayoutEngine::RectMake(Canvas::GetLeft(component), Canvas::GetTop(component),
				                                                 component->ActualWidth, component->ActualHeight);

				    onComponentLoaded(rect);
				});

			size_change_event_ = component_->SizeChanged += ref new Windows::UI::Xaml::SizeChangedEventHandler(
			    [this](Platform::Object ^ sender, SizeChangedEventArgs ^ e) {
				    auto component = getComponent();
				    auto rect = Titanium::LayoutEngine::RectMake(Canvas::GetLeft(component), Canvas::GetTop(component),
		...
		inline int toASCIIHexValue(char c) { return (c & 0xF) + (c < 'A' ? 0 : 9); }
		...
		Windows::UI::Color ViewBase::ColorForHexCode(const std::string& hexCode)
		{
			...
			if (length < 6) {
				value = ((value & 0xF000) << 16) | ((value & 0xFF00) << 12) | ((value & 0xFF0) << 8) |
				        ((value & 0xFF) << 4) | (value & 0xF);
			}
...
#define INSERT_WINDOWS_UI_COLOR(COLOR_NAME) \
	color_name_map.insert(std::make_pair(toLowerCase(#COLOR_NAME), Windows::UI::Colors::##COLOR_NAME));
