...
namespace UI
{
Label::Label(const JSContext& js_context) TITANIUM_NOEXCEPT : Titanium::UI::Label(js_context),
...
void Label::set_wordWrap(const JSValue& wordWrap) TITANIUM_NOEXCEPT
{
	...
	label__->TextWrapping =
	    is_wrapping ? Windows::UI::Xaml::TextWrapping::Wrap : Windows::UI::Xaml::TextWrapping::NoWrap;
}

void Label::set_fontFamily(const JSValue& family) TITANIUM_NOEXCEPT
{
	std::string text_string = static_cast<std::string>(family);
	label__->FontFamily = ref new Windows::UI::Xaml::Media::FontFamily(
	    ref new Platform::String(std::wstring(text_string.begin(), text_string.end()).c_str()));
}

void Label::set_fontSize(const JSValue& size) TITANIUM_NOEXCEPT { label__->FontSize = static_cast<double>(size); }
...
void Label::set_fontStyle(const JSValue& style) TITANIUM_NOEXCEPT
{
	...
	// TODO Windows supports Oblique:
	// http://msdn.microsoft.com/en-us/library/windows/apps/windows.ui.text.fontstyle.aspx
}
...
void Label::set_fontWeight(const JSValue& weight) TITANIUM_NOEXCEPT
{
	...
	// TODO Windows supports a large number of other weights:
	// http://msdn.microsoft.com/en-us/library/windows/apps/windows.ui.text.fontweights
}
...
void Label::enableEvent(const std::string& event_name) TITANIUM_NOEXCEPT
{
	...
	if (event_name == "click") {
		if (click_event_count_ == 0) {
			click_event_ = getComponent()->Tapped +=
