	...
	namespace UI
	{
		class ViewBase
		{
		   public:
			...
			virtual Windows::UI::Xaml::FrameworkElement ^ getComponent() const { return component_; }
		   ...
		   protected:
			...
			virtual std::string defaultHeight() const { return defaultHeight__; }

			virtual std::string defaultWidth() const { return defaultWidth__; }

			virtual void setDefaultHeight(const std::string& defaultHeight) { defaultHeight__ = defaultHeight; }

			virtual void setDefaultWidth(const std::string defaultWidth) { defaultWidth__ = defaultWidth; }
