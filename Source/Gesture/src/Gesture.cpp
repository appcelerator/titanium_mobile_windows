...
void Gesture::setupOrieintationListener()
{
	...
	if (!orientationEventSet_) {
		...
		const auto orientationchangeCallback = [this](DisplayInformation ^ sender, ::Platform::Object ^ args) {
			...
			obj.SetProperty("orientation",
			                ctx.CreateNumber(Titanium::UI::Constants::to_underlying_type(updateOrientation())));
			fireEvent("orientationchange", obj);
		};

		orientationchange_event_ = display->OrientationChanged += ref new Windows::Foundation::TypedEventHandler<
		    Windows::Graphics::Display::DisplayInformation ^, ::Platform::Object ^ >(orientationchangeCallback);
		orientationEventSet_ = true;
	}
}

Gesture::Gesture(const JSContext& js_context) TITANIUM_NOEXCEPT : Titanium::Gesture(js_context)
{
...
void Gesture::enableEvent(const std::string& event_name) TITANIUM_NOEXCEPT
{
	...
	if (event_name == "shake") {
		...
		using namespace std::placeholders;
		const auto updateCallback =
		    [current, self](Windows::Devices::Sensors::Accelerometer ^ sender,
			...
			const auto dispatchedCallback = [current, self]() {
				...
				if (self->previous_acceleromter_time_.UniversalTime > 0) {
					timestamp = static_cast<unsigned>(
					    (reading->Timestamp.UniversalTime - self->previous_acceleromter_time_.UniversalTime) / 10000.0);
			...
			if (current) {
				current->Dispatcher->RunAsync(Windows::UI::Core::CoreDispatcherPriority::Normal,
				                              ref new Windows::UI::Core::DispatchedHandler(dispatchedCallback));
			}
		};

		shake_event_ = accelerometer_->ReadingChanged += ref new Windows::Foundation::TypedEventHandler<
