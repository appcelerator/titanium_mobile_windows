...
{
	Platform::Platform(const JSContext& js_context) TITANIUM_NOEXCEPT : Titanium::PlatformModule(js_context),
	...
	unsigned Platform::availableMemory() const TITANIUM_NOEXCEPT { return 0; }
	...
	std::string Platform::locale() const TITANIUM_NOEXCEPT { return std::locale("").name(); }
	std::string Platform::macaddress() const TITANIUM_NOEXCEPT { return ""; }
	...
	std::string Platform::name() const TITANIUM_NOEXCEPT { return "windows"; }
	std::string Platform::netmask() const TITANIUM_NOEXCEPT { return ""; }
	std::string Platform::ostype() const TITANIUM_NOEXCEPT { return architecture(); }
	unsigned Platform::processorCount() const TITANIUM_NOEXCEPT { return 0; }
	std::string Platform::runtime() const TITANIUM_NOEXCEPT { return "javascriptcore"; }
	std::string Platform::username() const TITANIUM_NOEXCEPT
	{
		...
		if (UserInformation::NameAccessAllowed) {
			...
			concurrency::task<::Platform::String ^ >(UserInformation::GetDisplayNameAsync())
			    .then([&name, &event](concurrency::task<::Platform::String ^ > task) {
				          name = task.get();
				          event.set();
				      },
	...
	bool Platform::openURL(const std::string& url) TITANIUM_NOEXCEPT
	{
		...
		bool result = false;
		concurrency::event event;
		concurrency::task<bool>(Windows::System::Launcher::LaunchUriAsync(uri))
		    .then([&result, &event](concurrency::task<bool> task) {
			          result = task.get();
			          event.set();
			      },
		          concurrency::task_continuation_context::use_arbitrary());
	...
	bool Platform::is24HourTimeFormat() TITANIUM_NOEXCEPT { return false; }
