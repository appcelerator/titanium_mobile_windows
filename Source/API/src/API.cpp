...
{
	API::API(const JSContext& js_context) TITANIUM_NOEXCEPT : Titanium::API(js_context)
	{
		TITANIUM_LOG_DEBUG("API::ctor Initialize");
		connect();
	}

	API::API(const API& rhs, const std::vector<JSValue>& arguments) TITANIUM_NOEXCEPT : Titanium::API(rhs, arguments)
	{
	...
	void API::connect()
	{
		...
		auto get_file_task = create_task(
		    Windows::ApplicationModel::Package::Current->InstalledLocation->GetFileAsync("titanium_settings.ini"));

		// Read titanium_settings.ini file into String
		get_file_task.then([this](StorageFile ^ storage_file) { return FileIO::ReadTextAsync(storage_file); })
		    ...
		    .then([this](task<Platform::String ^ > antecedent) {
			    ...
			    for (auto ip_address : hosts) {
				    TITANIUM_LOG_DEBUG("API::connect: Trying to connect to log server: ", ip_address, ":",
				                       ConvertString(port));
				    ...
				    try {
					    ...
					    event connect_event;
					    task<void>(tcp_socket_->ConnectAsync(host, port))
					        .then([this, &connect_event](task<void> task) {
						              try {
							              task.get();
