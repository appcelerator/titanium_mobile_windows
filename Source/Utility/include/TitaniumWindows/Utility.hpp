...
namespace TitaniumWindows
{
	namespace Utility
	{
		...
		static std::vector<unsigned char> GetContentFromFile(Windows::Storage::StorageFile ^ file)
		{
			...
			concurrency::task<Windows::Storage::Streams::IBuffer ^ >(Windows::Storage::FileIO::ReadBufferAsync(file))
			    .then([&content, &event](concurrency::task<Windows::Storage::Streams::IBuffer ^ > task) {
				          try {
		...
		static unsigned GetMSecSinceEpoch(Windows::Foundation::DateTime d)
		{
			...
			std::tm tm = {cal->Second, cal->Minute, cal->Hour + (cal->Period == 1 ? 0 : 12),
			              cal->Day, cal->Month - 1, cal->Year - 1900,
			              0, 0, -1};
