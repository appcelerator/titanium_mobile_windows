	...
	class TITANIUMKIT_EXPORT Module : public JSExportObject, public JSExport<Module>
	{
	   public:
		/*!
		 ...
		 @result void
		 */
		virtual void addEventListener(const std::string& name, JSObject& callback,
		...
		/*!
		 ...
		 @result void
		 */
		virtual void removeEventListener(const std::string& name, JSObject& callback,
		                                 JSObject& this_object) TITANIUM_NOEXCEPT final;
	   ...
	   private:
		static unsigned eventListenerIndex(const JSObject& event_listener_list, const std::string& name,
