...
namespace Titanium
{
	...
	class TITANIUMKIT_EXPORT GlobalObject : public JSExportObject, public JSExport<GlobalObject>
	{
	   public:
		 ...
		 */
		virtual unsigned setTimeout(JSObject&& function,
		                            const std::chrono::milliseconds& delay) TITANIUM_NOEXCEPT final;
		...
		/*!
		 ...
		 @result Unique timer identifier (Number).
		 */
		virtual unsigned setInterval(JSObject&& function,
		                             const std::chrono::milliseconds& delay) TITANIUM_NOEXCEPT final;
...
#pragma warning(pop)
	   ...
	   protected:
		...
		/*!
		 ...
		 is called.
		 */
		virtual std::shared_ptr<Timer> CreateTimer(Callback_t callback,
		                                           const std::chrono::milliseconds& interval) const TITANIUM_NOEXCEPT;
	   ...
	   private:
		...
		void StartTimer(Callback_t&& callback, const unsigned& timerId,
