...
namespace Titanium
{
	TiModule::TiModule(const JSContext& js_context) TITANIUM_NOEXCEPT : Module(js_context),
	...
	void TiModule::JSExportInitialize()
	{
		...
		JSExport<TiModule>::AddValueProperty("userAgent", std::mem_fn(&TiModule::get_userAgent_ArgumentValidator),
		                                     std::mem_fn(&TiModule::set_userAgent_ArgumentValidator));
	...
	JSValue TiModule::getVersionArgumentValidator(const std::vector<JSValue>& arguments,
	...
	JSValue TiModule::getBuildDateArgumentValidator(const std::vector<JSValue>& arguments,
	                                                JSObject& this_object) TITANIUM_NOEXCEPT
	{
	...
	JSValue TiModule::getBuildHashArgumentValidator(const std::vector<JSValue>& arguments,
	                                                JSObject& this_object) TITANIUM_NOEXCEPT
	{
		return get_buildHash_ArgumentValidator();
	}

	JSValue TiModule::get_userAgent_ArgumentValidator() const TITANIUM_NOEXCEPT { return userAgent__; }

	JSValue TiModule::getUserAgentArgumentValidator(const std::vector<JSValue>& arguments,
	                                                JSObject& this_object) TITANIUM_NOEXCEPT
	{
		return get_userAgent_ArgumentValidator();
	}

	JSValue TiModule::setUserAgentArgumentValidator(const std::vector<JSValue>& arguments,
	                                                JSObject& this_object) TITANIUM_NOEXCEPT
	{
		if (arguments.size() < 1) {
	...
	void TiModule::setUserAgent(const JSValue& userAgent) TITANIUM_NOEXCEPT { userAgent__ = userAgent; }
	...
	JSValue TiModule::createBufferArgumentValidator(const std::vector<JSValue>& arguments,
	                                                JSObject& this_object) TITANIUM_NOEXCEPT
	{
