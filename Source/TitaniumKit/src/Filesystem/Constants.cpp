	...
	namespace Filesystem
	{
		...
		MODE Constants::to_MODE(const std::string& modeName) TITANIUM_NOEXCEPT
		{
			...
			if (position != map.end()) {
				...
				TITANIUM_LOG_WARN("Constants::to_MODE: Titanium::Filesystem::MODE with name '", modeName,
				                  "' does not exist");
			}
		...
		std::underlying_type<MODE>::type Constants::to_underlying_type(const std::unordered_set<MODE>& mode_set)
		    TITANIUM_NOEXCEPT
		{
