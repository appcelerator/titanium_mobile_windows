...
namespace Titanium
{
	namespace UI
	{
		ListItem::ListItem(const JSContext& js_context) TITANIUM_NOEXCEPT : Module(js_context),
		...
		LIST_ACCESSORY_TYPE ListItem::get_accessoryType() const TITANIUM_NOEXCEPT { return accessoryType__; }
		...
		void ListItem::JSExportInitialize()
		{
			...
			JSExport<ListItem>::AddValueProperty("accessoryType",
			                                     std::mem_fn(&ListItem::getAccessoryTypeArgumentValidator),
			                                     std::mem_fn(&ListItem::setAccessoryTypeArgumentValidator));
		}

		JSValue ListItem::getAccessoryTypeArgumentValidator() const TITANIUM_NOEXCEPT
		{
			return get_context().CreateNumber(
			    static_cast<std::underlying_type<LIST_ACCESSORY_TYPE>::type>(get_accessoryType()));
		}

		bool ListItem::setAccessoryTypeArgumentValidator(const JSValue& argument) TITANIUM_NOEXCEPT
		{
			TITANIUM_ASSERT(argument.IsNumber());
			accessoryType__ = Constants::to_LIST_ACCESSORY_TYPE(
			    static_cast<std::underlying_type<LIST_ACCESSORY_TYPE>::type>(argument));
