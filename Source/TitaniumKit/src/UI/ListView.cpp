...
namespace Titanium
{
	namespace UI
	{
		...
		LIST_ITEM_TEMPLATE ListView::get_defaultItemTemplate() const TITANIUM_NOEXCEPT { return defaultItemTemplate__; }
		...
		void ListView::JSExportInitialize()
		{
			...
			JSExport<ListView>::AddValueProperty("defaultItemTemplate",
			                                     std::mem_fn(&ListView::getDefaultItemTemplateArgumentValidator),
			                                     std::mem_fn(&ListView::setDefaultItemTemplateArgumentValidator));
		}

		JSValue ListView::getDefaultItemTemplateArgumentValidator() const TITANIUM_NOEXCEPT
		{
			return get_context().CreateNumber(
			    static_cast<std::underlying_type<LIST_ITEM_TEMPLATE>::type>(get_defaultItemTemplate()));
		}

		bool ListView::setDefaultItemTemplateArgumentValidator(const JSValue& argument) TITANIUM_NOEXCEPT
		{
			TITANIUM_ASSERT(argument.IsNumber());
			defaultItemTemplate__ =
			    Constants::to_LIST_ITEM_TEMPLATE(static_cast<std::underlying_type<LIST_ITEM_TEMPLATE>::type>(argument));
