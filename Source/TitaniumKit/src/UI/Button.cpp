...
namespace Titanium
{
	namespace UI
	{
		Button::Button(const JSContext& js_context) TITANIUM_NOEXCEPT : View(js_context),
		...
		Button::~Button() TITANIUM_NOEXCEPT { TITANIUM_LOG_DEBUG("Button:: dtor ", this); }

		JSValue Button::get_title() const TITANIUM_NOEXCEPT { return title__; }
		...
		void Button::JSExportInitialize()
		{
			...
			JSExport<Button>::AddValueProperty("title", std::mem_fn(&Button::get_title),
			                                   std::mem_fn(&Button::setTitleArgumentValidator));
		}
