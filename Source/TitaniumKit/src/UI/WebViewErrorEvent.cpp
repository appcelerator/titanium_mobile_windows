...
namespace Titanium
{
	namespace UI
	{
		...
		WebViewErrorEvent::WebViewErrorEvent(const WebViewErrorEvent& rhs,
		...
		URL_ERROR WebViewErrorEvent::get_error() const TITANIUM_NOEXCEPT { return error__; }
		...
		void WebViewErrorEvent::JSExportInitialize()
		{
			...
			JSExport<WebViewErrorEvent>::AddValueProperty("error",
			                                              std::mem_fn(&WebViewErrorEvent::getErrorArgumentValidator),
			                                              std::mem_fn(&WebViewErrorEvent::setErrorArgumentValidator));
		}
