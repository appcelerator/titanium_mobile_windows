...
namespace Titanium
{
	namespace UI
	{
		ImageView::ImageView(const JSContext& js_context) TITANIUM_NOEXCEPT : View(js_context),
		...
		JSValue ImageView::get_image() const TITANIUM_NOEXCEPT { return image__; }

		void ImageView::set_image(const JSValue& title) TITANIUM_NOEXCEPT { image__ = title; }
		...
		void ImageView::JSExportInitialize()
		{
			...
			JSExport<ImageView>::AddValueProperty("image", std::mem_fn(&ImageView::get_image),
			                                      std::mem_fn(&ImageView::setImageArgumentValidator));
		}
