...
namespace Titanium
{
	namespace UI
	{
		Picker::Picker(const JSContext& js_context) TITANIUM_NOEXCEPT : View(js_context), type__(PICKER_TYPE::PLAIN) {}
		...
		PICKER_TYPE Picker::get_type() const TITANIUM_NOEXCEPT { return type__; }
		...
		void Picker::JSExportInitialize()
		{
			...
			JSExport<Picker>::AddValueProperty("type", std::mem_fn(&Picker::getTypeArgumentValidator),
			                                   std::mem_fn(&Picker::setTypeArgumentValidator));
		}
