...
namespace Titanium
{
	namespace UI
	{
		Animation::Animation(const JSContext& js_context) TITANIUM_NOEXCEPT : Module(js_context),
		...
		ANIMATION_CURVE Animation::get_curve() const TITANIUM_NOEXCEPT { return curve__; }

		void Animation::set_curve(const ANIMATION_CURVE& curve) TITANIUM_NOEXCEPT { curve__ = curve; }
		...
		void Animation::JSExportInitialize()
		{
			...
			JSExport<Animation>::AddValueProperty("curve", std::mem_fn(&Animation::getCurveArgumentValidator),
			                                      std::mem_fn(&Animation::setCurveArgumentValidator));
		}
