...
namespace Titanium
{
	namespace UI
	{
		Window::Window(const JSContext& js_context) TITANIUM_NOEXCEPT : View(js_context),
		...
		Window::~Window() TITANIUM_NOEXCEPT { TITANIUM_LOG_DEBUG("Window:: dtor ", this); }
		...
		std::unordered_set<EXTEND_EDGE> Window::get_extendEdges() const TITANIUM_NOEXCEPT { return extendEdges__; }
		...
		JSValue Window::openArgumentValidator(const std::vector<JSValue>& arguments,
