...
namespace Titanium
{
	Module::Module(const JSContext& js_context) TITANIUM_NOEXCEPT : JSExportObject(js_context),
	      ...
	      event_listener_map__(get_context().CreateObject())
	{
		...
		if (arguments.size() >= 1) {
			...
			if (_0.IsObject()) {
				...
				TITANIUM_LOG_DEBUG("Module:: ctor 2 has ", properties.GetPropertyNames().GetCount(), " properties for ",
				                   this);
			}
		}
	}

	Module::~Module() TITANIUM_NOEXCEPT { TITANIUM_LOG_DEBUG("Module:: dtor ", this); }
	...
	void Module::addEventListener(const std::string& name, JSObject& callback, JSObject& this_object) TITANIUM_NOEXCEPT
	{
		...
		if (event_listener_index > 0) {
			TITANIUM_LOG_WARN("Module::addEventListener: event listener already added at index ", event_listener_index,
			                  " for event '", name, "'");
		} else {
			...
			TITANIUM_LOG_DEBUG("Module::addEventListener: add listener at index ", callback_list_index, " for event '",
			                   name, "' for ", this);
			event_listener_list.SetProperty(static_cast<unsigned>(callback_list_index), callback_payload);
		}
	}

	void Module::removeEventListener(const std::string& name, JSObject& callback,
	                                 JSObject& this_object) TITANIUM_NOEXCEPT
	{
		...
		if (event_listener_count > 0) {
			...
			TITANIUM_LOG_DEBUG("Module::removeEventListener: remove listener at index ", event_listener_index,
			                   " for event '", name, "' for ", this);
			...
			if (!deleted) {
				...
				TITANIUM_LOG_ERROR("Module::removeEventListener: event listener at index ", event_listener_index,
				                   " for event '", name, "' was not removed");
	...
	unsigned Module::eventListenerIndex(const JSObject& event_listener_list, const std::string& name,
	                                    JSObject& callback) TITANIUM_NOEXCEPT
	{
		unsigned index = 0;
		bool found = false;
		const auto event_listener_count = event_listener_list.GetPropertyNames().GetCount();
		for (unsigned i = 0; i < event_listener_count; ++i) {
			...
			if (callback == callback_property) {
				...
				if (found) {
					// This is a TitaniumKit logic error.
					TITANIUM_LOG_ERROR("Module::eventListenerIndex: multiple identical listeners found for event = '",
					                   name, "'");
				}
	...
	void Module::JSExportInitialize()
	{
		...
		JSExport<Module>::AddFunctionProperty("addEventListener",
		                                      std::mem_fn(&Module::addEventListenerArgumentValidator));
		JSExport<Module>::AddFunctionProperty("removeEventListener",
		                                      std::mem_fn(&Module::removeEventListenerArgumentValidator));
		JSExport<Module>::AddFunctionProperty("applyProperties",
		                                      std::mem_fn(&Module::applyPropertiesArgumentValidator));
