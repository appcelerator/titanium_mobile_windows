...
namespace Titanium
{
	...
	{
		struct ComputedSize doHorizontalLayout(std::vector<struct Element*> children, double width, double height,
		                                       bool isWidthSize, bool isHeightSize)
		{
			...
			for (i = 0; i < len; i++) {
				...
				// Pedro
				measuredWidth = widthLayoutCoefficients.x1 * width +
				                widthLayoutCoefficients.x2 * (width - (isWidthSize ? 0 : runningWidth)) +
				                widthLayoutCoefficients.x3;
				// measuredWidth = widthLayoutCoefficients.x1 * width + widthLayoutCoefficients.x2 * (width -
				// runningWidth) + widthLayoutCoefficients.x3;

				measuredHeight = heightLayoutCoefficients.x2 == 0
				                     ? heightLayoutCoefficients.x1 * height + heightLayoutCoefficients.x3
				                     : NAN;

				if (isNaN(measuredWidth) || isNaN(heightLayoutCoefficients.x1)) {
					childSize =
					    layoutNode(child, isNaN(measuredWidth) ? width : measuredWidth - (*child).borderLeftWidth -
					...
					isNaN(heightLayoutCoefficients.x1) &&
					...
					if (heightLayoutCoefficients.x2 != 0 && !isNaN(heightLayoutCoefficients.x2)) {
						std::clog << "Child of width SIZE and height FILL detected in a horizontal layout. Performance "
						             "degradation may occur." << std::endl;
				...
				measuredSandboxWidth = (*child).measuredSandboxWidth =
				    sandboxWidthLayoutCoefficients.x1 * width + sandboxWidthLayoutCoefficients.x2 + measuredWidth;
			...
			for (i = 0; i < len; i++) {
				...
				for (j = 0; j < row.size(); j++) {
					...
					if (isNaN(measuredHeight)) {
						(*child).measuredHeight = measuredHeight =
						    heightLayoutCoefficients.x1 * height +
					...
					if (topLayoutCoefficients.x2 != 0) {
						...
						(*child).measuredTop = measuredTop = topLayoutCoefficients.x1 * height +
						                                     topLayoutCoefficients.x3 * measuredHeight +
						                                     topLayoutCoefficients.x4 + runningHeight;
					}

					(*child).measuredSandboxHeight = measuredSandboxHeight =
					    sandboxHeightLayoutCoefficients.x1 * height + sandboxHeightLayoutCoefficients.x2 +
					    measuredHeight + measuredTop - runningHeight;
			...
			for (i = 0; i < len; i++) {
				...
				for (j = 0; j < rowLen; j++) {
					...
					if (std::find(deferredTopCalculations.begin(), deferredTopCalculations.end(), child) !=
					    deferredTopCalculations.end()) {
						measuredHeight = (*child).measuredHeight;
						topLayoutCoefficients = (*child).layoutCoefficients.top;
						(*child).measuredTop =
						    topLayoutCoefficients.x1 * height + topLayoutCoefficients.x2 * rowHeight +
		...
		static void setDefaultHorizontalWidthType(struct LayoutProperties layoutProperties,
		...
		static void setDefaultHorizontalHeightType(struct LayoutProperties layoutProperties,
