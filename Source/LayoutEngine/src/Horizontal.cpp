                                       ...
                                       bool isWidthSize, bool isHeightSize)
{
	...
	for (i = 0; i < len; i++) {
		...
		// measuredWidth = widthLayoutCoefficients.x1 * width + widthLayoutCoefficients.x2 * (width - runningWidth) +
		// widthLayoutCoefficients.x3;

		measuredHeight =
		    heightLayoutCoefficients.x2 == 0 ? heightLayoutCoefficients.x1 * height + heightLayoutCoefficients.x3 : NAN;
	...
	for (i = 0; i < len; i++) {
		...
		for (j = 0; j < row.size(); j++) {
			...
			if (isNaN(measuredHeight)) {
				(*child).measuredHeight = measuredHeight = heightLayoutCoefficients.x1 * height +
				                                           heightLayoutCoefficients.x2 * (height - runningHeight) +
				                                           heightLayoutCoefficients.x3;
			}
	...
	for (i = 0; i < len; i++) {
		...
		for (j = 0; j < rowLen; j++) {
			...
			if (std::find(deferredTopCalculations.begin(), deferredTopCalculations.end(), child) !=
				...
				(*child).measuredTop = topLayoutCoefficients.x1 * height + topLayoutCoefficients.x2 * rowHeight +
				                       topLayoutCoefficients.x3 * measuredHeight + topLayoutCoefficients.x4 +
				                       runningHeight;
			}
		}
		runningHeight += rowHeight;
