...
namespace Titanium
{
	...
	{
		struct ComputedSize doCompositeLayout(std::vector<struct Element*> children, double width, double height,
		                                      bool isWidthSize, bool isHeightSize)
		{
			...
			struct ThreeCoefficients widthLayoutCoefficients, heightLayoutCoefficients, sandboxWidthLayoutCoefficients,
			    sandboxHeightLayoutCoefficients, leftLayoutCoefficients, minWidthLayoutCoefficients,
			    minHeightLayoutCoefficients;
			...
			for (i = 0; i < len; i++) {
				...
				if (!(isNaN(minWidthLayoutCoefficients.x1))) {
					measuredWidth =
					    std::max(measuredWidth, minWidthLayoutCoefficients.x1 * width + minWidthLayoutCoefficients.x2);
				}

				measuredHeight = heightLayoutCoefficients.x1 * height + heightLayoutCoefficients.x2;
				if (!(isNaN(minHeightLayoutCoefficients.x1))) {
					measuredHeight = std::max(measuredHeight,
					                          minHeightLayoutCoefficients.x1 * height + minHeightLayoutCoefficients.x2);
				}

				childSize = layoutNode(child, isNaN(measuredWidth) ? width : measuredWidth - (*child).borderLeftWidth -
				                                                                 (*child).borderRightWidth,
				                       isNaN(measuredHeight) ? height : measuredHeight - (*child).borderTopWidth -
				                                                            (*child).borderBottomWidth,
				                       isNaN(measuredWidth), isNaN(measuredHeight));
				...
				if (isNaN(measuredHeight)) {
					...
					if (!(isNaN(minHeightLayoutCoefficients.x1))) {
						measuredHeight = std::max(
						    measuredHeight, minHeightLayoutCoefficients.x1 * height + minHeightLayoutCoefficients.x2);
					}
				}
				...
				if (isWidthSize && leftLayoutCoefficients.x1 != 0) {
					...
					measuredLeft = leftLayoutCoefficients.x1 * width + leftLayoutCoefficients.x2 * measuredWidth +
					               leftLayoutCoefficients.x3;
				}
				if (isHeightSize && topLayoutCoefficients.x1 != 0) {
					deferredTopCalculations.push_back(child);
				} else {
					measuredTop = topLayoutCoefficients.x1 * height + topLayoutCoefficients.x2 * measuredHeight +
					              topLayoutCoefficients.x3;
				}

				(*child).measuredSandboxWidth = measuredSandboxWidth =
				    sandboxWidthLayoutCoefficients.x1 * height + sandboxWidthLayoutCoefficients.x2 + measuredWidth +
				    (isNaN(measuredLeft) ? 0 : measuredLeft);
				(*child).measuredSandboxHeight = measuredSandboxHeight =
				    sandboxHeightLayoutCoefficients.x1 * height + sandboxHeightLayoutCoefficients.x2 + measuredHeight +
				    (isNaN(measuredTop) ? 0 : measuredTop);
			...
			for (i = 0; i < len; i++) {
				...
				(*child).measuredLeft = measuredLeft = leftLayoutCoefficients.x1 * computedSize.width +
				                                       leftLayoutCoefficients.x2 * measuredWidth +
				                                       leftLayoutCoefficients.x3;
				(*child).measuredSandboxWidth = measuredSandboxWidth = sandboxWidthLayoutCoefficients.x1 * height +
				                                                       sandboxWidthLayoutCoefficients.x2 +
				                                                       (*child).measuredWidth + measuredLeft;
			...
			for (i = 0; i < len; i++) {
				...
				(*child).measuredTop = measuredTop = topLayoutCoefficients.x1 * computedSize.height +
				                                     topLayoutCoefficients.x2 * measuredHeight +
				                                     topLayoutCoefficients.x3;
				(*child).measuredSandboxHeight = measuredSandboxHeight = sandboxHeightLayoutCoefficients.x1 * height +
				                                                         sandboxHeightLayoutCoefficients.x2 +
				                                                         (*child).measuredHeight + measuredTop;
		...
		static void setDefaultCompositeWidthType(struct LayoutProperties layoutProperties,
		                                         enum ValueType* measuredWidthType)
		{
		...
		static void setDefaultCompositeHeightType(struct LayoutProperties layoutProperties,
		                                          enum ValueType* measuredHeightType)
		{
