                                     ...
                                     bool isWidthSize, bool isHeightSize)
{
	...
	for (i = 0; i < len; i++) {
		...
		measuredHeight = heightLayoutCoefficients.x1 * height + heightLayoutCoefficients.x2 * (height - runningHeight) +
	...
	// Calculate the preliminary sandbox widths (missing left, since one of these widths may end up impacting all the
	// lefts)
	...
	for (i = 0; i < len; i++) {
		...
		measuredSandboxWidth = (*child).measuredSandboxWidth =
		    sandboxWidthLayoutCoefficients.x1 * width + sandboxWidthLayoutCoefficients.x2 + (*child).measuredWidth;
		measuredSandboxWidth > computedSize.width&&(computedSize.width = measuredSandboxWidth);
	}
