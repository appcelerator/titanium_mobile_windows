...
namespace Titanium
{
	...
	{
		struct ComputedSize doVerticalLayout(std::vector<struct Element*> children, double width, double height,
		                                     bool isWidthSize, bool isHeightSize)
		{
			...
			for (i = 0; i < len; i++) {
				...
				measuredHeight = heightLayoutCoefficients.x1 * height +
				...
				if (isWidthSize && leftLayoutCoefficients.x1 != 0) {
					...
					measuredLeft = (*child).measuredLeft = leftLayoutCoefficients.x1 * width +
			...
			// Calculate the preliminary sandbox widths (missing left, since one of these widths may end up impacting
			// all the lefts)
			...
			for (i = 0; i < len; i++) {
				...
				measuredSandboxWidth = (*child).measuredSandboxWidth = sandboxWidthLayoutCoefficients.x1 * width +
			...
			for (i = 0; i < len; i++) {
				...
				measuredSandboxWidth > computedSize.width&&(computedSize.width = measuredSandboxWidth);
				measuredLeft = (*child).measuredLeft = leftLayoutCoefficients.x1 * computedSize.width +
				                                       leftLayoutCoefficients.x2 * measuredWidth +
				                                       leftLayoutCoefficients.x3;
		...
		static void setDefaultVerticalWidthType(struct LayoutProperties layoutProperties,
		...
		static void setDefaultVerticalHeightType(struct LayoutProperties layoutProperties,
