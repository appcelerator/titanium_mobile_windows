...
namespace TitaniumWindows
{
	...
	class TITANIUMWINDOWS_GLOBAL_EXPORT Timer final : public Titanium::GlobalObject::Timer,
	                                                  public std::enable_shared_from_this<Timer>
	{
	   public:
		    ...
		    : Titanium::GlobalObject::Timer(callback, interval), callback__(callback)
		{
			...
			std::chrono::duration<std::chrono::nanoseconds::rep, std::ratio_multiply<std::ratio<100>, std::nano>>
			    timer_interval_ticks = interval;
		...
		virtual void Start() TITANIUM_NOEXCEPT override final
		{
			...
			std::call_once(dispatcher_timer_once_flag__, [this] {
				...
				auto callback = std::bind(
				    [](const std::weak_ptr<Timer>& weakThis, Platform::Object ^ sender, Platform::Object ^ event) {
				...
				this->event_registration_token__ = this->dispatcher_timer__->Tick +=
				    ref new Windows::Foundation::EventHandler<Platform::Object ^ >(callback);
			});
	...
	std::shared_ptr<Titanium::GlobalObject::Timer> GlobalObject::CreateTimer(
	...
	GlobalObject::GlobalObject(const JSContext& js_context) TITANIUM_NOEXCEPT : Titanium::GlobalObject(js_context)
	{
