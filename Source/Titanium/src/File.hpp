	...
	namespace Filesystem
	{
		...
		class TITANIUMWINDOWS_EXPORT File final : public Titanium::Filesystem::File, public JSExport<File>
		{
		   ...
		   private:
			...
			bool isFolder() const { return folder_ != nullptr; }
			...
			Windows::Storage::FileProperties::BasicProperties ^
			    getStorageProperties(Windows::Storage::IStorageItem ^ file) const;
			...
			Windows::Storage::Streams::IBuffer ^
			    getBufferFromBytes(unsigned char* data, std::size_t size, bool append,
			                       Windows::Storage::StorageFile ^ appendingFile) {
			    ...
			    Windows::Storage::Streams::IBuffer ^
			    getBufferFromString(const std::string& string, bool append,
			                        Windows::Storage::StorageFile ^ appendingFile) {
				    using namespace Windows::Storage;
				    const auto writer = ref new Streams::DataWriter(ref new Streams::InMemoryRandomAccessStream());
				    if (append) {
			    ...
			    std::size_t writeContentFromFile(Windows::Storage::Streams::DataWriter ^ writer,
