...
namespace TitaniumWindows
{
	...
	{
		File::File(const JSContext& js_context) TITANIUM_NOEXCEPT : Titanium::Filesystem::File(js_context)
		{
		...
		File::~File() { TITANIUM_LOG_DEBUG("TitaniumWindows::Filesystem::File::dtor"); }
		...
		bool File::createDirectory(const std::string& path)
		{
			...
			task<StorageFolder ^ >(folder->CreateFolderAsync(TitaniumWindows::Utility::ConvertString(desiredName)))
			    .then([&result, &event](task<StorageFolder ^ > task) {
				          try {
					          task.get();
					          result = true;
		...
		FileProperties::BasicProperties ^ File::getStorageProperties(IStorageItem ^ file) const
		{
			try {
				...
				concurrency::event event;
				task<FileProperties::BasicProperties ^ >(file->GetBasicPropertiesAsync())
				    .then([&properties, &event](task<FileProperties::BasicProperties ^ > task) {
					          try {
						          properties = task.get();
					          } catch (Platform::COMException ^ ex) {
						          TITANIUM_LOG_DEBUG(TitaniumWindows::Utility::ConvertString(ex->Message));
		...
		Windows::Storage::StorageFolder ^ File::getFolderFromPathSync(Platform::String ^ filename) const
		{
			...
			concurrency::event event;
			task<StorageFolder ^ >(Windows::Storage::StorageFolder::GetFolderFromPathAsync(filename))
			    .then([&storageFolder, &event](task<StorageFolder ^ > task) {
				          try {
		...
		Windows::Storage::StorageFile ^ File::getFileFromPathSync(Platform::String ^ filename) const
		{
			...
			concurrency::event event;
			task<StorageFile ^ >(Windows::Storage::StorageFile::GetFileFromPathAsync(filename))
			    .then([&storageFile, &event](task<StorageFile ^ > task) {
				          try {
		...
		std::string File::get_name() const TITANIUM_NOEXCEPT { return name_; }
		...
		bool File::get_remoteBackup() const TITANIUM_NOEXCEPT { return false; }
		bool File::get_symbolicLink() const TITANIUM_NOEXCEPT { return false; }
		...
		bool File::copy(const std::string& destinationPath) TITANIUM_NOEXCEPT
		{
			...
			concurrency::event event;
			bool result = false;
			task<void>(file_->CopyAndReplaceAsync(fileToReplace))
			    .then([&result, &event](task<void> task) {
				          try {
					          task.get();
					          result = true;
				          } catch (Platform::COMException ^ ex) {
		...
		bool File::deleteDirectory(bool recursive) TITANIUM_NOEXCEPT { return deleteFile(); }
		...
		bool File::exists() TITANIUM_NOEXCEPT { return (getStorageItem() != nullptr); }
		...
		std::vector<JSValue> File::getDirectoryListing() TITANIUM_NOEXCEPT
		{
			...
			task<IVectorView<StorageFolder ^ > ^ >(folder_->GetFoldersAsync())
			    .then([&ctx, &filenames, &folderEvent](task<IVectorView<StorageFolder ^ > ^ > task) {
				          try {
					          auto folders = task.get();
					          std::for_each(begin(folders), end(folders), [&ctx, &filenames](StorageFolder ^ folder) {
						          filenames.push_back(
						              ctx.CreateString(TitaniumWindows::Utility::ConvertString(folder->Name)));
						      });
				          } catch (Platform::COMException ^ ex) {
					          TITANIUM_LOG_DEBUG(TitaniumWindows::Utility::ConvertString(ex->Message));
				          }
			...
			task<IVectorView<StorageFile ^ > ^ >(folder_->GetFilesAsync()).then(
			    [&ctx, &filenames, &fileEvent](task<IVectorView<StorageFile ^ > ^ > task) {
				    try {
					    auto files = task.get();
		...
		bool File::move(const std::string& newpath) TITANIUM_NOEXCEPT
		{
			...
			bool result = false;
			concurrency::event event;
			task<void>(this->file_->MoveAndReplaceAsync(fileToReplace))
			    .then([&result, &event](task<void> task) {
				          try {
					          task.get();
					          result = true;
		...
		bool File::rename(const std::string& desiredName) TITANIUM_NOEXCEPT
		{
			...
			concurrency::event event;
			bool result = false;
			task<void>(item->RenameAsync(TitaniumWindows::Utility::ConvertString(desiredName)))
			    .then([&result, &event](task<void> task) {
				          try {
					          task.get();
					          result = true;
		...
		std::string File::resolve() TITANIUM_NOEXCEPT { return path_; }
		unsigned File::spaceAvailable() TITANIUM_NOEXCEPT
		{
			...
			uint64 freeSpace;
			concurrency::event event;
			task<IMap<Platform::String ^, Platform::Object ^ > ^ >(prop->RetrievePropertiesAsync(propertiesName))
			    .then([&freeSpace, &event](task<IMap<Platform::String ^, Platform::Object ^ > ^ > task) {
				          try {
		...
		bool File::write(const JSValue& data, bool append) TITANIUM_NOEXCEPT
		{
			...
			bool result = false;
			concurrency::event event;
			task<void>(FileIO::WriteBufferAsync(file_, buffer))
			    .then([&result, &event](task<void> task) {
				          try {
					          task.get();
					          result = true;
